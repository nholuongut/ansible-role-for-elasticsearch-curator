...
On RedHat/CentOS, make sure you have the EPEL repository configured, so the `python-pip` package can be installed. You can install the EPEL repo by simply adding `nholuong.repo-epel` to your playbook's roles.
...
## Dependencies
  ...
  - nholuong.repo-epel (RedHat/CentOS only)

## Example Playbook

    - hosts: search
      roles:
        - { role: nholuong.elasticsearch-curator }
